...
const hasTodoProperty = (requestQuery) => requestQuery.todo !== undefined;
...
app.get("/todos/", async (request, response) => {
  ...
  const booksArray = await db.all(getTodosQuery);
...
app.get("/todos/:todoId/", async (request, response) => {
  ...
  const getTodoQuery = `
        ...
        WHERE 
            id = ${todoId};
    `;
  const todoEl = await db.get(getTodoQuery);
  response.send(todoEl);
});
